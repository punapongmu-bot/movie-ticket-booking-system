...
## ภาคผนวก B: ลำดับการทำงานของโปรแกรม (Flow)

1. เปิดโปรแกรม → `LoginFrame` เปิดขึ้นมา ให้สมัครสมาชิก / เข้าสู่ระบบ
2. ล็อกอินสำเร็จ → `MovieSelectFrame` แสดงหนัง 3 เรื่อง → คลิกเลือกเรื่อง
3. `HallSelectFrame` แสดง 3 โรง (ถูก/กลาง/แพง) ของหนังเรื่องนั้น → เลือกโรง
4. `ShowtimeSelectFrame` เลือกวันฉาย (3 วัน) แล้วเลือกรอบเวลา (3 รอบ)
5. `SeatMapFrame` โหลดสถานะที่นั่งจาก `seats.csv` ของรอบฉายนั้น แสดงผังพร้อมสีตามสถานะ และทำเครื่องหมายแถว VIP (F)
6. ผู้ใช้คลิกเลือกได้หลายที่นั่ง → ระบบคำนวณราคาต่อที่นั่งและราคารวมอัตโนมัติ
7. กด "ยืนยัน" → `PaymentFrame` สรุปรายการ + ราคารวม → กดยืนยันจ่ายเงิน (จำลอง)
8. บันทึกทุกที่นั่งที่เลือกลง `bookings.csv` ด้วย `booking_id` เดียวกัน และอัปเดต `seats.csv` เป็นจองแล้ว
9. เปิด `TicketFrame` แสดงตั๋วหนังจำลองที่รวมทุกที่นั่งของการจองนั้น

---

## ภาคผนวก C: ประเด็นที่กลุ่มต้องตกลงกันก่อนเริ่มเขียนโค้ด

- [ ] ภาค Star Wars ที่จะใช้ (ระบุให้ชัดเจนก่อนใส่ใน `movies.csv`)
- [ ] วันที่จริงที่จะใช้เป็น 3 วันฉาย
- [ ] จำกัดจำนวนที่นั่งสูงสุดต่อการจอง 1 ครั้งหรือไม่
- [ ] เก็บรหัสผ่านแบบ plain text หรือเข้ารหัสเบื้องต้น
- [ ] ใช้ `JFrame` แยกหน้าต่างต่อขั้นตอน หรือใช้ `JFrame` เดียวกับ `CardLayout` สลับหน้าจอ (แนะนำ `CardLayout` สำหรับมือใหม่)
- [ ] ดีไซน์หน้าตา GUI คร่าว ๆ (สี/โทนธีม, ขนาดหน้าต่าง, ฟอนต์, สีแยกที่นั่ง VIP)
